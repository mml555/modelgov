...
- [ ] Set `OIDC_AUDIENCE` when operator SSO is enabled (`AI_GUARD_PRODUCTION=true` enforces this at boot)
...
Prometheus metrics are exposed at `GET /metrics` (`METRICS_ENABLED=true`, default on): request rate/errors/latency (`http_requests_total`, `http_request_duration_seconds`), pg pool saturation (`pg_pool_connections_total` / `_idle`, `pg_pool_clients_waiting`), Node process defaults, and domain (business) counters: `ai_guard_chat_requests_total{feature,decision,status}`, `ai_guard_chat_cost_usd_total{feature}`, `ai_guard_chat_fallbacks_total{feature}`, `ai_guard_budget_blocks_total{feature}`, and `ai_guard_safety_blocks_total{feature}`.
...
Alert on: 5xx rate, p95 of `http_request_duration_seconds`, sustained `pg_pool_clients_waiting > 0`, budget-block rate (`rate(ai_guard_budget_blocks_total[5m])`), and provider-fallback rate (`rate(ai_guard_chat_fallbacks_total[5m])`).

Logs are structured JSON (pino). Set verbosity with `LOG_LEVEL` (`fatal|error|warn|info|debug|trace|silent`, default `info`). Every log line, the `requestId` in error responses, and the `x-ai-guard-request-id` response header share one id per request (`reqId` in logs), so a client-reported id pivots straight to that request's logs.
