...
| **Container registry** | Pull `ghcr.io/<org>/modelgov-api:v1.2.0` (pin digest in values) |
...
```yaml
image:
  tag: v1.2.0   # or sha256:...
```

Verify artifacts after release:

```bash
scripts/verify-release-artifacts.sh v1.2.0 your-org/Modelgov
```
