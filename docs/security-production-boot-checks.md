...
When `AI_GUARD_PRODUCTION=true`, the API refuses to boot on unsafe configuration.
Use `pnpm ai-guard doctor production` for an offline check before deploy.

| Error | Severity | Fix |
| --- | --- | --- |
| Known dev API key | fail | Replace `sk-ai-guard-api-local` / `smoke-test-key` with a random secret |
...
| `AI_GUARD_BEHIND_PROXY=true` without `TRUST_PROXY` | fail | Set `TRUST_PROXY` to LB CIDR or hop count |
