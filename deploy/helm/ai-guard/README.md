...
```bash
helm install ai-guard ./deploy/helm/ai-guard \
  -f deploy/helm/ai-guard/values-multitenant.yaml \
  ...
```
...
| --- | --- | --- | --- | --- |
| `selfhost` | `values-selfhost.yaml` | Flat (default) | Single `ai-guard.yaml` file | One org, self-hosted gateway |
...
Set `deployProfile: selfhost|multitenant` (or `AI_GUARD_DEPLOY_PROFILE` in
compose) and run `pnpm ai-guard doctor production` to verify posture.
