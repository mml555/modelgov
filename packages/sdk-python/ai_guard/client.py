"""Synchronous Ai-Guard API client.

A thin, typed HTTP client over the Ai-Guard REST API. Policy enforcement is
...
* structured exceptions (:class:`~ai_guard.errors.AiGuardError` and
  subclasses) carrying the API error envelope;
* a streaming generator (:meth:`AiGuardClient.chat_stream`) over SSE.
"""
...
class AiGuardClient:
    """Synchronous client for the Ai-Guard API.

    Args:
        base_url: Base URL of the Ai-Guard API (e.g. ``http://localhost:3000``).
    ...
    The client is a context manager; use ``with AiGuardClient(...) as c:`` to
    ...
    def __enter__(self) -> "AiGuardClient":
        return self
        ...
        *,
        ...
        SSE framing assumption:
            ...
            * ``chunk["delta"]`` or ``chunk["content"]`` or ``chunk["text"]``
              (simpler shapes the Ai-Guard API may emit).
        ...
        Raises:
            SafetyBlockedError / PolicyBlockedError / AiGuardError: on a non-2xx
    ...
    def _raise_for_status(response: httpx.Response) -> None:
        """Map a non-2xx response to the appropriate AiGuardError subclass."""
        try:
            body = response.json()
        except (json.JSONDecodeError, ValueError):
            body = {}
        ...
        raise AiGuardError(response.status_code, code, body)
...
def _parse_sse_line(line: str) -> Any:
    ...
    the extracted text chunk otherwise. See :meth:`AiGuardClient.chat_stream`
