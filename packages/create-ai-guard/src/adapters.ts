...
// boundary: the host app authenticates/authorizes, THEN calls Ai-Guard. TS
// frameworks use @ai-guard/sdk; FastAPI uses the Python ai-guard-sdk.
...
const BOUNDARY = [
  ...
  "// product action (auth + RBAC). Ai-Guard decides whether the AI call is",
  "// allowed (budget / model access / safety). Do your auth check first.",
].join("\n");

function tsClient(): string {
  return `import { createAiGuardClient } from "@ai-guard/sdk";

// One client for the whole app. Point AI_GUARD_URL at your Ai-Guard deployment.
...
# Boundary: your app authenticates/authorizes the user first; Ai-Guard enforces
...
// NOTE: `@ai-guard/sdk` and `ai-guard-sdk` are not yet published to npm/PyPI, so
...
export function adapterFor(framework: Framework, template: Template): AdapterOutput {
  switch (framework) {
    case "nextjs":
      return {
        files: { "lib/ai-guard.ts": tsClient(), "app/api/ai/route.ts": nextRoute(template) },
        installHint: "npm i @ai-guard/sdk (when published; until then install from source — see docs/self-host.md)",
      };
    case "express":
      return {
        files: { "src/ai-guard.ts": tsClient(), "src/routes/ai.ts": expressRoute(template) },
        installHint: "npm i express, plus @ai-guard/sdk (when published; until then install from source — see docs/self-host.md)",
      };
    case "fastify":
      return {
        files: { "src/ai-guard.ts": tsClient(), "src/routes/ai.ts": fastifyRoute(template) },
        installHint: "npm i fastify, plus @ai-guard/sdk (when published; until then install from source — see docs/self-host.md)",
      };
    case "fastapi":
      return {
        files: { "app/routes/ai.py": fastapiRoute(template) },
        installHint: "pip install fastapi, plus ai-guard-sdk (when published; until then install from source — see docs/self-host.md)",
      };
    case "none":
